...
from gaishisim.utils.simulators import DataSimulator
from .simulation_batch_processing import get_true_tracts_batch
from .simulation_utils import generate_mutation_rate_map
from gaishisim.utils.simulators.InferRelate import InferRelate
from gaishisim.utils.simulators.InferTskit import InferTskit
...
class MsprimeSimulator(DataSimulator):
    ...
    def __init__(
        ...
        get_phased_and_unphased_tract_file: bool=True,
        ...
        mutation_model : str
            Mutation model to be used to for msprime simulations, currently binary ('binary') and Jukes-Cantor ('JC69', default) are directly supported.
        rec_map: str
        mut_map: str
        shuffle_rec_map: bool
        ...
        self.get_phased_and_unphased_tract_file = get_phased_and_unphased_tract_file

        self.mutation_model = mutation_model
        self.rec_map = rec_map
        self.mut_map = mut_map
        self.shuffle_rec_map = shuffle_rec_map

        self.L_mut_map = L_mut_map
        self.cv_mut_map = cv_mut_map



        self.true_tracts_batch_processing = true_tracts_batch_processing

        if rec_map is not None:
            # recombination maps, e.g. "palsson.CO.maps.avg.tsv"
            stored_rec_map = pd.read_csv(rec_map, sep="\t", comment="#")
            stored_rec_map_positions = [0] + stored_rec_map["position"].tolist()
            stored_rec_map_rates = stored_rec_map["rate"].tolist()
            rec_rate_map = msprime.RateMap(
                position=stored_rec_map_positions, rate=stored_rec_map_rates
            )
            sliced_ratemap = rec_rate_map.slice(0, seq_len, trim=True)
            self.rec_map = sliced_ratemap

        
        self.create_relate_output = create_relate_output
        self.relate_config = relate_config or {}
        self.tsinfer_config = tsinfer_config or {}
        self.tsinfer_check_ancestral_state = tsinfer_check_ancestral_state

        self.create_tsinfer_output = create_tsinfer_output
    ...
    def run(self, rep: int = None, seed: int = None) -> list[dict[str, str]]:
        ...
        # mutation rates
        if self.mut_map is not None:
            # check whether it could be a path
            mean_block_length = self.L_mut_map
            mean_rate = self.mut_rate
            cv = self.cv_mut_map
            gamma_shape = 2.5
            normal_sd = 1e-9
            if "." not in self.mut_map:
                if "exponential" in self.mut_map:
                    if self.L_mut_map == "list":
                        L_choices = [10000, 100000, 1000000]
                        mean_block_length = np.random.choice(L_choices)
                    if self.cv_mut_map == "list":
                        cv_choices = [0.15, 0.25]
                        cv = np.random.choice(cv_choices)

                rate_map = generate_mutation_rate_map(
                    genome_length=self.seq_len,
                    mean_block_length=mean_block_length,
                    # default (Cousins) exponential
                    mode=self.mut_map,
                ...
                mut_rate_object = rate_map
            else:
                # load mutation rate map, currently not available
                pass
        else:
            mut_rate_object = self.mut_rate

        # recombination rates
        if self.rec_map is not None:

            rec_rate_object = self.rec_map
            if self.shuffle_rec_map:
                new_rates = rec_rate_object.rate
                new_rates = np.random.permutation(new_rates)
                rate_map_new = msprime.RateMap(
                    position=rec_rate_object.position, rate=new_rates
                )
                rec_rate_object = rate_map_new

        else:
            rec_rate_object = self.rec_rate

        ts = msprime.sim_ancestry(
            recombination_rate=rec_rate_object,
        ...
        ts = msprime.sim_mutations(
            ts,
            rate=mut_rate_object,
            random_seed=seed,
            model=mutation_model_map[self.mutation_model](),
        )
        ...
        if not self.get_phased_and_unphased_tract_file:

            if not self.true_tracts_batch_processing:
                ...
                # res_df = get_true_tracts_batch(ts, self.tgt_id, self.src_id, self.ploidy, self.is_phased, bed_file)
                get_true_tracts_batch(
                    ts, self.tgt_id, self.src_id, self.ploidy, self.is_phased, bed_file
                )

        else:
            bed_file_phased = os.path.join(output_dir, f"{output_prefix}.true.tracts.bed")
            bed_file_unphased = os.path.join(output_dir, f"{output_prefix}.unphased.true.tracts.bed")
            file_paths["bed_file_phased"] = bed_file_phased
            file_paths["bed_file_unphased"] = bed_file_unphased


            both_bed_files = [[True, bed_file_phased], [False, bed_file_unphased]]
            for bed_file_elem in both_bed_files:

                if not self.true_tracts_batch_processing:
                    true_tracts = self._get_true_tracts(
                        ts, self.tgt_id, self.src_id, self.ploidy, bed_file_elem[0]
                    )
                    true_tracts = pr.from_string(true_tracts).merge(by="Sample")

                    if true_tracts.empty:
                        open(bed_file_elem[1], "w").close()
                    else:
                        true_tracts.to_csv(bed_file_elem[1], sep="\t", header=False)
                else:
                    # res_df = get_true_tracts_batch(ts, self.tgt_id, self.src_id, self.ploidy, bed_file_elem[0], bed_file_elem[1])
                    get_true_tracts_batch(
                        ts, self.tgt_id, self.src_id, self.ploidy, bed_file_elem[0], bed_file_elem[1]
                    )




        if self.create_relate_output:
            self._run_relate_inference(vcf_file, output_prefix, seed, output_dir)


        if self.create_tsinfer_output:
            self._run_tsinfer_inference(vcf_file, output_prefix, ts_file=ts_file)
    ...
    def _run_relate_inference(self, vcf_file, output_prefix, seed, output_dir, Ne=None):

        relate_prefix = os.path.join(output_dir, f"{output_prefix}.relate")

        infer = InferRelate(
            vcf_file=vcf_file,
            output_prefix=relate_prefix,
            recombination_rate=self.rec_rate,
            sequence_length=self.seq_len,
            mutation_rate=self.mut_rate,

            # optional overrides
            demes_file=self.demo_model_file,
            Ne=Ne,  # lets InferRelate decide (true demes or estimate)

            mode=self.relate_config.get("mode", "parallel"),
            #mode=self.relate_config.get("mode", "standard"),
            threads=self.relate_config.get("threads", 1),
            memory=self.relate_config.get("memory", 5),
        )

        infer.run()


    def _run_tsinfer_inference(self, vcf_file, output_prefix, ts_file=None):
        from gaishisim.utils.simulators.InferTskit import InferTskit
